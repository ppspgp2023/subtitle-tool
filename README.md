...
- ✂️ **指定时间段**：可只对某一段（如 `00:30:00-00:45:00`）生成字幕，省时省钱
...
如果只想给电影的某一段做字幕（例如测试翻译质量、或只需要其中一节），可以指定时间段——**只有那一段会被听写和翻译，速度更快、费用更省**。

运行时程序会多问一句，直接回车＝整部；或输入时间段：
...
- 分隔符：`-`、`~`、`到`、`至` 均可（如 `30:00到45:00`）

**要点**：

- 生成的字幕**时间轴仍对应整部电影**——在完整视频里播放时，只有那一段会显示字幕，其余时间为空。也就是「文件覆盖整部时长，但只有那段有内容」。
- 为避免覆盖整部版本，只做一段时输出文件名会带时间段后缀，如 `电影名.段003000-004500.srt`。想让播放器自动加载，把它重命名成与视频**同名**的 `.srt` 即可。
